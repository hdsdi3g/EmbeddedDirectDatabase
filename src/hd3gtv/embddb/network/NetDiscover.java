...
public class NetDiscover {
	...
	NetDiscover(PoolManager manager) throws IOException {
		...
		listeners = manager.getAddressMaster().getBroadcastAddresses().map(addr -> {
			...
			try {
				...
				log.error("Can't create socket listener", e);
				return null;
			}
		}).filter(listener -> {
	...
	void startRegularSend() {
		json_host_public_listened_addrs = manager.getSimpleGson().toJson(manager.getListenedServerAddress().filter(socket -> {
			return manager.getAddressMaster().isPublicAndPhysicalAddress(socket.getAddress());
		}).collect(Collectors.toList())).getBytes(Protocol.UTF8);
		uuid_string = manager.getUUIDRef().toString().getBytes(Protocol.UTF8);
		
		sch_future = Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(getPacketSender(), 3, 20, TimeUnit.SECONDS);
	}
	...
	private void readDatagram(byte[] raw_datas) throws IOException, GeneralSecurityException {
		...
		ArrayList<InetSocketAddress> socket_list = manager.getSimpleGson().fromJson(json_addrs, type_ArrayList_InetSocketAddress);
